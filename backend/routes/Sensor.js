...
router.get("/readings/latest", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Error fetching latest readings" });
  }
});

// Get historical sensor data by date range
router.get("/readings/range", async (req, res) => {
  const { start, end, page = 1, pageSize = 15 } = req.query;
  try {
    const data = await SensorData.find({
      timestamp: {
        $gte: new Date(start),
    ...
    res
      .status(500)
      .json({ success: false, error: "Error fetching historical data" });
  }
});
...
router.get("/readings/hourly", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Error fetching hourly data" });
  }
});
...
router.get("/status", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Error checking server status" });
  }
});
